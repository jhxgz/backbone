...
def evaluate_rag_model(args, feature_extractor, tokenizer, model, eval_df):
    ...
    for idx in tqdm(range(len(eval_df))):
        ...
        if args.features_path is not None:
            # [修改] 优化 numpy 到 tensor 的转换，消除 UserWarning
            # 原代码: encoder_last_hidden_state = torch.FloatTensor([features[image_id][()]])
            encoder_last_hidden_state = torch.from_numpy(features[image_id][()]).unsqueeze(0)
...
def load_model(args, checkpoint_path):
    ...
    # 从检查点的state_dict中推断fusion_dim
    # 注意：不再需要推断 text_hidden，因为我们会直接传入正确的 text_encoder
    ...
    if state_dict_path:
        # 简单尝试读取 state_dict 以获取 fusion_dim (如果需要)
        # 如果 config 中已经正确保存了 fusion_dim，这一步其实也是可选的
        pass

    # 设置config中的fusion_dim (如果有必要，通常 config.json 里是对的)
    # config.fusion_dim = ...

    # --- [关键修改] 先加载CLIP text encoder ---
    print(f"Loading CLIP Text Encoder: {args.encoder_name} ...")
    ...
    # --- [关键修改] 加载模型时显式传入 text_encoder ---
    # 这样 SmallCap 初始化时就能读取到 clip_text_encoder.config.hidden_size (512)
    # 从而正确初始化 PromptProjector (512 -> 768)
    print(f"Loading SmallCap model from {checkpoint_path} ...")
    model = AutoModel.from_pretrained(
        checkpoint_path,
        config=config,
        ignore_mismatched_sizes=True,  # 允许形状不匹配
        text_encoder=clip_text_encoder,  # <=== 必须传入这个！
        text_tokenizer=clip_text_tokenizer  # 顺便传入 tokenizer
    )
    model.config = config

    # 再次确保 attach (虽然 from_pretrained 可能已经处理了)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--images_dir", type=str, default="data/images/train/",
