...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_chn, out_chn, kernel_size, dilation):
        super(DepthwiseSeparableConv, self).__init__()
        self.depthwise = nn.Conv2d(in_chn, in_chn, kernel_size=kernel_size, padding=dilation, dilation=dilation,
                                   groups=in_chn, bias=False)
        self.pointwise = nn.Conv2d(in_chn, out_chn, kernel_size=1, bias=False)
        self.bn = nn.GroupNorm(8, out_chn)
        self.relu = nn.ReLU()
...
class MSGDC(nn.Module):
    def __init__(self, in_chn, dilation1=1, dilation2=3, dilation3=5, kernel_size=3, stride=1, padding='same'):
        ...
        self.cov1 = DepthwiseSeparableConv(in_chn, in_chn, kernel_size, dilation1)
        self.cov2 = DepthwiseSeparableConv(in_chn, in_chn, kernel_size, dilation2)
        self.cov3 = DepthwiseSeparableConv(in_chn, in_chn, kernel_size, dilation3)
        self.norm = nn.LayerNorm(in_chn, eps=1e-6)
        self.act1 = RPReLU(in_chn)
        self.act2 = RPReLU(in_chn)
        self.act3 = RPReLU(in_chn)
        self.msgdc_dropout = nn.Dropout(p=0.2)
